    ...
    <template #mobile-content>
      <div class="flex-1 flex items-center justify-center p-4">
        <div class="max-w-md mx-auto text-center space-y-6">
          ...
          <!-- 메시지 -->
          <div class="space-y-3">
            <h2 class="text-xl font-semibold text-gray-900">정말 삭제하시겠습니까?</h2>
            <p class="text-gray-600">
              <span class="font-medium text-gray-900">{{ currentMember?.name }}</span>님의 정보를 삭제합니다.
            </p>
          ...
          <!-- 회원 정보 요약 -->
          <div v-if="currentMember" class="bg-gray-50 rounded-lg p-4 text-left space-y-2">
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">이름:</span>
              <span class="text-sm font-medium text-gray-900">{{ currentMember.name }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">이메일:</span>
              <span class="text-sm font-medium text-gray-900">{{ currentMember.email }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">전화번호:</span>
              <span class="text-sm font-medium text-gray-900">{{ currentMember.phone }}</span>
            </div>
          </div>
        </div>
      </div>
